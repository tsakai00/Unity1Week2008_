...
namespace Lib.Sound
{
    ...
    public class SoundPlayer : MonoBehaviour
    {
        ...
        /// 再生
        /// </summary>
        public void Play(string key, float fadeSec = 0.0f, float pitch = 1.0f, bool isLoop = false)
        {
            var clip = GetAudioClip(key);
            var src  = GetAudioSource();
            src?.Play(clip, 1.0f, fadeSec, pitch, isLoop, this);
        }
        ...
        public void PlayCrossFade(string key, float fadeSec = FADE_SEC, bool isLoop = true)
        {
            ...
            if(IsPlaying())
            {
                // 他を再生中ならクロスフェード
                Stop(fadeSec);
                Play(key, fadeSec, 1.0f, isLoop);
            }
            else
            {
                // 通常再生
                Play(key, 0.0f, 1.0f, isLoop);
            }
        }
